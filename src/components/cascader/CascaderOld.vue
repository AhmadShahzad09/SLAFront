<template>
  <div
    ...
    :class="[
      'ods-cascader',
      realSize && `ods-cascader--${realSize}`,
    ...
    <ods-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="ods-input__icon ods-icon-circle-close"
    ...
    <div v-if="multiple" class="ods-cascader__tags">
      <ods-tag
      ...
      <input
        ...
        type="text"
        class="ods-cascader__search-input"
    ...
    <transition name="ods-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['ods-popper', 'ods-cascader__dropdown', popperClass]">
        <ods-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></ods-cascader-panel>
        <ods-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="ods-cascader__suggestion-panel"
          view-class="ods-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="ods-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="ods-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </ods-scrollbar>
      </div>
    </transition>
...
import OdsInput from '@/components/packages/input/Input'
import OdsTag from '@/components/packages/tag/Tag'
import OdsScrollbar from '@/components/packages/scrollbar/Scrollbar'
import OdsCascaderPanel from '@/components/packages/cascader-panel/CascaderPanel'
import AriaUtils from '@/addons/utils/aria-utils'
import { t } from '@/addons/locale'
import { isEqual, isEmpty, kebabCase } from '@/addons/utils/util'
import { isUndefined, isFunction } from '@/addons/utils/types'
import { isDef } from '@/addons/utils/shared'
...
export default {
  ...
  computed: {
    realSize () {
      const _odsFormItemSize = (this.odsFormItem || {}).odsFormItemSize
      return this.size || _odsFormItemSize || (this.$ONESAIT || {}).size
    },
    tagSize () {
  ...
  methods: {
    ...
    focusFirstNode () {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.ods-cascader__suggestion-item')
        } else {
          const firstMenu = popper.querySelector('.ods-cascader-menu')
          firstNode = firstMenu.querySelector('.ods-cascader-node[tabindex="-1"]')
        }
        if (firstNode) {
          firstNode.focus()
    ...
    updateStyle () {
      ...
      const inputInner = $el.querySelector('.ods-input__inner')
      if (!inputInner) return
      const tags = $el.querySelector('.ods-cascader__tags')
      let suggestionPanelEl = null
      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.ods-cascader__suggestion-list')
